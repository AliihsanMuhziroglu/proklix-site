...
export function WorkflowSection({ dict }: Props) {
  ...
  return (
    <section id="processes" className="overflow-hidden bg-navy-950 py-10 text-white sm:py-14">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <h2 className="text-xl font-bold tracking-tight sm:text-2xl lg:text-3xl">{workflow.title}</h2>
          <p className="mx-auto mt-3 max-w-[640px] text-sm leading-relaxed text-slate-300 sm:mt-4 sm:text-base">
            {workflow.subtitle}
          </p>
        </div>
        ...
        <ol
          key={tabs[activeTab]?.id ?? activeTab}
          className="relative mt-6 space-y-0 sm:mt-8"
        >
          <div
            className="absolute bottom-4 left-[15px] top-4 w-px bg-white/15 md:hidden"
            aria-hidden="true"
          />
            ...
            {steps.map((step, i) => (
              <li key={step.title} className="relative flex flex-1 flex-col md:min-w-0 md:flex-[1_1_13%]">
                <div className="relative pl-10 md:pl-0 md:pt-0">
                  <span
                    className={`absolute left-0 top-3 flex h-7 w-7 items-center justify-center rounded-full border text-[11px] font-bold md:relative md:left-auto md:top-auto md:mb-2 md:inline-flex ${stepNumberClass(i)}`}
                  >
                    {i + 1}
                  </span>
                  <div className={`rounded-xl border p-3 sm:p-3.5 ${stepCardClass(i)}`}>
                    ...
                    <h3
                      className={`mt-2 text-[13px] font-semibold leading-snug ${
                        i === TELEGRAM_STEP_INDEX ? "text-blue-100" : "text-white"
                      }`}
                    >
                      {step.title}
                    </h3>
                    <p className="mt-1 text-[12px] leading-relaxed text-slate-300">{step.description}</p>
                  </div>
                </div>
                {i < steps.length - 1 && (
                  <span
                    className="my-1.5 flex justify-center text-slate-500 md:absolute md:-right-3 md:top-1/2 md:my-0 md:-translate-y-1/2 md:text-xs"
                    aria-hidden="true"
                  >
        ...
        <p className="mx-auto mt-6 max-w-[640px] text-center text-[13px] leading-relaxed text-slate-400 sm:mt-8 sm:text-sm">
        ...
        <div className="mt-4 text-center">
          <a
            ...
            className="inline-flex min-h-[44px] items-center justify-center rounded-2xl border border-white/20 bg-white/5 px-5 py-2.5 text-sm font-semibold text-white transition hover:border-white/30 hover:bg-white/10"
          >
